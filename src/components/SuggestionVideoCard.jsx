  ...
  return (
    <Link to={`/video/${video.videoId}`}>
      <div className='flex mb-3'>
        <div className='relative h-24 lg:h-20 xl:h-24 w-40 min-w-[160px] lg:w-32 lg:min-w-[120px]
        xl:w-40 xl:min-w-[168px] rounded-xl bg-slate-800 overflow-hidden'>
          <img className='h-full w-full object-cover' src={video?.thumbnails?.[0]?.url} alt='Thumbnail'/>
          {video.lengthSeconds && (<VideoLength time={video?.lengthSeconds}/>)}
        </div>
        <div className='flex flex-col ml-3 overflow-hidden'>
            <span className='text-sm lg:text-xs xl:text-sm font-bold text-white line-clamp-2'>{video?.title}</span>
            <span className='text-[12px] lg:text-[10px] xl:text-[12px] font-semibold mt-2 text-white/[0.7] flex items-center'>
              {video?.author?.title}
              {video?.author?.badges[0]?.type && video?.author?.badges[0]?.type ==="VERIFIED_CHANNEL" &&(
